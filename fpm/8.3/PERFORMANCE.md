## PHP 8.3 FPM - Performance Findings

Why the OPcache, JIT, FPM pool and `mpm_event` values in these images are what they are.

Measurements were taken against `ianmgg/php83fpm:latest` / `:cli` (PHP 8.3.33, `linux/arm64`) with no application code deployed — so per-worker figures are floors, not Flarum numbers. Everything labelled *measured* is reproducible with the commands at the end; everything else is sourced and linked.
...
| OPcache ships as an extension on 8.3 | Enabled via `docker-php-ext-opcache.ini`; can still be disabled by ini, which is what the dev images do. On 8.5 it is compiled into the binary |
| `opcache.file_cache_read_only` needs 8.5 | Lets a warm file cache be baked at build time and mounted read-only. Not available on 8.3 |
...
docker run --rm -e PHP_OPCACHE_JIT=off ianmgg/php83fpm:cli php -r '
